...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var RegularNewsImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        regular_news_file_frame = wp.media.frames.regular_news_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' ),
          },
          state : 'tpiw-insert-image',
              states : [
                  new RegularNewsImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        regular_news_file_frame.on( 'select', function() {

          var state = regular_news_file_frame.state('tpiw-insert-image');
