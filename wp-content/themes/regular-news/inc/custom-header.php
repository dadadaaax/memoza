 ...
 *
 * @uses regular_news_header_style()
 */
function regular_news_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'regular_news_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'regular_news_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'regular-news' ),
		),
	) );
}
add_action( 'after_setup_theme', 'regular_news_custom_header_setup' );

if ( ! function_exists( 'regular_news_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see regular_news_custom_header_setup().
	 */
	function regular_news_header_style() {
		...
		wp_add_inline_style( 'regular-news-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'regular_news_header_style', 10 );