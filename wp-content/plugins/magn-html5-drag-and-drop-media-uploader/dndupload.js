...
function magnCreateUploader() {
    ...
    var uploader = new qq.FileUploader({
        ...
        onProgress: function(id, fileName, loaded, total){
          ...
          var   procent = Math.round(100*(loaded/total));
            jQuery('.qq-upload-drop-area').html("<div style=\"text-align:center; color:white;\"><h2>Ładowanie pliku "+procent + "%<h2></div>");
        ...
        onCancel: function(id, fileName){
            jQuery('#dndmedia_status').text("Ładowanie przerwane");
...
function dndmediaProcessCompletedFileUpload(id, fileName, response)
{
    ...
    if (response.url)
    {
        ...
        //ZMIANA - dodaj dane o zauploadowanych zalaczniku do formy jakko hidden input, tak, by po przeslaniu formy 
        // post-zalacznik mogl zostac podczepiony do postu-wpisu stworzonego wlasnie przez uzytkownika. Post-zalacznik musi stac sie synem postu-wpisu.
        //TODO: co jesli bedzie kilka ID? 
        
        //dopiero teraz pokażmy pola edycji qpy
         jQuery('#qpa-upload-right-panel').show();
        
        
         
         //schowajmy instukcje ładownia
          jQuery('#load_instrukcja0').hide();
          jQuery('#load_instrukcja1').hide();
          jQuery('#load_instrukcja2').hide();
           jQuery('#old_choosefile').hide();
               
      jQuery('#qpa_new_post_form').append('<input type=\"hidden" name=\"dnd_attachment_id\" value=\"'+response.attachment_id+'\" />');
      jQuery('#qpa_new_post_form').append('<input type=\"hidden" name=\"dnd_attachment_url\" value=\"'+response.url+'\" />');
      jQuery('#qpa_new_post_form').append('<input type=\"hidden" name=\"dnd_file\" value=\"'+response.file+'\" />');  
            
            
        var markup = new String();
		
        response.name = response.url.replace(/^.*[\\\/]/, '');
		//ZMIANA - usuniety insert
        markup = "<div class='dndmedia_file_row'>" +
        ...
        //ZMIANA
        jQuery('#drop-box-jsupload-gmail').html("<img src=\""+response.url+"\" alt=\"\" class=\"qpa_tmp_img\" />");
         jQuery('#qpa-upload-left-panel').css({ "background-color": "white", "padding": "5px 5px 5px 5px", "-webkit-box-shadow": "0 2px 5px rgba(0,0,0,0.5)", "box-shadow": "0 2px 5px rgba(0,0,0,0.5)"});
        ...
        jQuery('#dndmedia_status').text("Błąd ładowania.");
    }
}
...
function initDnDExtraTools()
{
    jQuery("#dndmedia_importurl").click( function() {
        var result = window.prompt("Podaj adres URL do zaimportowania","");
        if (result!=undefined)
        {
