...
function widget_dndmedia_init() {
	...
	// create custom plugin settings menu
        ...
        //TO MOZE BY TERAZ NIEPOTRZEBNE, BO NIE CHCEMY pos tronie administracyjnej żadnego menu;
	...
	function dndmedia_create_menu() {
		...
		add_action( 'init', 'register_dndmedia_settings' );
		add_action( 'init', 'dndmedia_add_meta_boxes' );
	...
	function dndmedia_add_meta_boxes()
	{
		...
		if (TRUE) {
			...
			foreach ( $post_types as $post_type ) {
				...
				add_meta_box( 'dndmedia_metabox', 'Magn DragY and Drop Upload', 'dndmedia_show_metabox_ui', $post_type, 'normal', 'high' );
			}	
			//WYWALIC
                       var_dump($post_types);
			...
			add_meta_box( 'dndmedia_metabox', 'Magn DragX and Drop Upload', 'dndmedia_show_metabox_ui', 'post', 'normal', 'high' );
		}
...
add_action('init', 'dndmedia_admin_head');
function dndmedia_admin_head()
{
	...
	wp_enqueue_script("jquery-ui-core");
        //ZMIANA
        wp_enqueue_style('jquery-style', '/wp-content/plugins/magn-html5-drag-and-drop-media-uploader/jquery-ui.css');
	//wp_enqueue_style('jquery-style', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.2/themes/smoothness/jquery-ui.css');

        //ZMIANA - UWAGA - jak te wszystkie skrypty ładować tylko kiedy user jest zalogowany i chce edytować page????
        
           $js_file = plugins_url('/js/jquery.min.js', __FILE__);
           
     //  wp_register_script('jquery.min.js', $js_file, array('jquery'), '1.0', FALSE);
      // wp_enqueue_script('jquery.min.js');
           
           
         //  $js_file = plugins_url('/js/jquery.validate.min.js', __FILE__);
           
          // wp_register_script('jquery.validate.min.js', $js_file, array('jquery'), '1.0', FALSE);
...
add_action('wp_ajax_nopriv_dndmedia', 'ajax_dndmedia_callback');
...
// TO TU ZOSTAŁ UTWORZONY ZAŁĄCZNIK w bibliotece, czy POST. Teraz musimy sprawić, by jego rodzicem był POST (WPIS), który jeszcze nie jest utworzony
                              //zapisujemy jego dane do sesji - dla reCaptcha
                                $_SESSION['dnd_attachment_id']=$attach_id;
                                $_SESSION['dnd_attachment_url']=$url;
                                $_SESSION['dnd_file']=$file;
...
add_action('wp_ajax_nopriv_dndmedia_importurl', 'ajax_dndmedia_importurl_callback');
